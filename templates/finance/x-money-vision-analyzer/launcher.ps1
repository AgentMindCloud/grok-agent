...
#   grok-agent run     x-money-vision-analyzer
#
...
#
# Optional flags:
#   -Port <int>     Port to run Streamlit on (default 8504 — Tool #1=8501,
#                   Tool #2=8502, Tool #3=8503 planned, Tool #4=8504; all
#                   four X Money tools coexist on the same Windows machine)
...
#    $env:LOCALAPPDATA\grok-agent\x-money-vision-analyzer
#    AND the receipts/ subfolder if missing, then initialises the SQLite
#    schema via data.store.init_db().
# 4. Launches Streamlit headless on http://localhost:<Port> and opens
#    the URL in your default browser (unless -NoBrowser).
#
# Constitution Articles II + III + V apply: this analyzer is the ONLY
# Constitution-permitted cross-tool writer (Article III), every batch
# import passes through the Article II consent gate, and every screen
# carries NOT FINANCIAL ADVICE / NOT TAX ADVICE banners (Article V).
# The launcher itself only sets up + starts the app — it never moves
# money, never imports without consent, never bypasses the consent gate.
...
function Show-Banner {
    ...
    Write-Host "    Drag, drop, parse, import."                          -ForegroundColor DarkCyan
    Write-Host "    Built for xAI, X, Grok and the ecosystem community. ❤️"                     -ForegroundColor DarkCyan
    Write-Host "    Writes to Tool #1's SQLite (consent-gated only)."    -ForegroundColor DarkCyan
    Write-Host "  =====================================================" -ForegroundColor DarkCyan
    Write-Host ""
...
# ---------------------------------------------------------------------
# AppData folder + receipts/ subfolder + SQLite schema (idempotent)
# ---------------------------------------------------------------------
...
$appData     = Join-Path $env:LOCALAPPDATA "grok-agent\x-money-vision-analyzer"
$receiptsDir = Join-Path $appData "receipts"

foreach ($p in @($appData, $receiptsDir)) {
    if (-not (Test-Path $p)) {
