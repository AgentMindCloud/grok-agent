...
4. **v2.15 manifest validator with kind-specific rules** — Pydantic enforcement that `vision-analyzer` requires `grok.vision: true` and declares `import_to_companion_dashboard`; `creator-payout-optimizer` declares its cross-tool readers; etc. Effort: **M**.
