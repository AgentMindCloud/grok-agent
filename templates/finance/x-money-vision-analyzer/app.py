...
# table — the real receipts / parsed_items tables live in
# data/store.py::init_db. Call it on import so a direct `streamlit run app.py`
# (without the launcher) doesn't fail with "no such table: receipts". init_db
# is idempotent and re-runs migrations cheaply.
try:
    ...
    # when the first tab queries. Streamlit will render a clean trace.
    pass
